...
foreach ($file in $mdFiles) {
    ...
    if ($date) {
        ...
        $sortedDateFileMap[$date] += $file.BaseName
    } else {
...
foreach ($key in ($sortedDateFileMap.Keys | Sort-Object {[datetime]::Parse($_)} -Descending)) {
    $finalDateFileMap[$key] = @($sortedDateFileMap[$key] | Sort-Object -Descending)
}

# 将有序的字典转换为 JSON 格式
$jsonOutput = $finalDateFileMap | ConvertTo-Json -Depth 2
